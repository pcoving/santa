...
def pack_z(pres_layer, coords_layer, pres_layer2):
    
    Nlayer = len(pres_layer)
    zmax = max([pres_layer[pid][2] for pid in range(Nlayer)])
    
    Nlayer2 = len(pres_layer2)
    coords_layer2 = [[-1,-1,-1]]*Nlayer2
    packed2 = [False]*Nlayer2

    # just check if everyone can be lazily packed on top of someone else
    for pid2 in range(Nlayer2):
        if not packed2[pid2]:
            max_density, max_pres, max_pid = 0., None, None
            for idx0, idx1, idx2 in permutations(range(3), 3):
                pres = [pres_layer2[pid2][idx0], pres_layer2[pid2][idx1], pres_layer2[pid2][idx2]]
                for pid in range(len(pres_layer)):
                    density = fits(pres, pres_layer[pid], zmax)
            ...
            packed2[pid2] = True
            pres_layer2[pid2] = max_pres[:]
            coords_layer2[pid2] = [coords_layer[max_pid][0], coords_layer[max_pid][1], zmax-pres_layer2[pid2][2]+coords_layer[max_pid][2]]
            for i in range(2):
                assert pres_layer2[pid2][i] + coords_layer2[pid2][i] <= sleigh_size[i] 
                assert pres_layer2[pid2][i] + coords_layer2[pid2][i] >= 0

            # add sub rectangles (similar to tree algo)
            '''
            if (pres_layer[max_pid][0] - pres_layer2[pid2][0]) > (pres_layer[max_pid][1] - pres_layer2[pid2][1]):
                pres_layer.append([pres_layer[max_pid][0] - pres_layer2[pid2][0], pres_layer[max_pid][1], pres_layer[max_pid][2]])
                coords_layer.append([coords_layer[max_pid][0] + pres_layer2[pid2][0], coords_layer[max_pid][1], coords_layer[max_pid][2]])
                
                pres_layer.append([pres_layer2[pid2][0], pres_layer[max_pid][1] - pres_layer2[pid2][1], pres_layer[max_pid][2]])
                coords_layer.append([coords_layer[max_pid][0], coords_layer[max_pid][1] + pres_layer2[pid2][1], coords_layer[max_pid][2]])
            else:
                pres_layer.append([pres_layer[max_pid][0], pres_layer[max_pid][1] - pres_layer2[pid2][1], pres_layer[max_pid][2]])
                coords_layer.append([coords_layer[max_pid][0], coords_layer[max_pid][1] + pres_layer2[pid2][1], coords_layer[max_pid][2]])
                
                pres_layer.append([pres_layer2[pid2][0], pres_layer2[pid2][1], pres_layer[max_pid][2]])
                coords_layer.append([coords_layer[max_pid][0] + pres_layer2[pid2][0], coords_layer[max_pid][1], coords_layer[max_pid][2]])
            '''
            del coords_layer[max_pid], pres_layer[max_pid]
            
    return pres_layer2, coords_layer2
...
while True:
    ...
    pid_current += len(pres_layer)
    
    Nlayer2 = 2
    while (pack_z(pres_layer[:], coords_layer[:], presents[pid_current:(pid_current+Nlayer2)])):
        Nlayer2 *= 2
    
    Nupper, Nlower = Nlayer2, Nlayer2/2
    
    while (Nupper - Nlower > 1):
        Nlayer2 = (Nupper + Nlower)/2
        if (pack_z(pres_layer[:], coords_layer[:], presents[pid_current:(pid_current+Nlayer2)])):
            Nlower = Nlayer2
        else:
            Nupper = Nlayer2
    ...
    Nlayer2 = Nlower    
    if Nlayer2 >= 2:
        pres_layer2, coords_layer2 = pack_z(pres_layer[:], coords_layer[:], presents[pid_current:(pid_current+Nlayer2)])
        presents[pid_current:pid_current+len(pres_layer2)] = pres_layer2
        coords[pid_current:pid_current+len(pres_layer2)] = coords_layer2
        pid_current += len(pres_layer2)
        pres_layer += pres_layer2
        coords_layer += coords_layer2
        Nlayer += Nlayer2
    
    layer_height = max([x[2]+c[2] for x,c in zip(pres_layer, coords_layer)]) - min([c[2] for c in coords_layer])
    ...
    if pid_current >= 50000: #Np:
        print '2*layer_x3:', 2.*layer_x3
        for pid in range(pid_current, Np):
            coords[pid] = [0,0,layer_x3]
            layer_x3 += presents[pid][2]
        print '2*layer_x3:', 2.*layer_x3
        break
